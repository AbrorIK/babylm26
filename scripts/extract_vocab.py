...
def main():
    ...
    content = collections.Counter({w: n for w, n in counts.items()
                                   if is_content_word(w, majority.get(w))})
    print(f"{len(content):,} content word forms")

    kept = [w for w, _ in content.most_common()][:args.top_n]
    ...
    print(f"kept {len(kept):,} word forms "
          f"(freq {content[kept[-1]]:,}..{content[kept[0]]:,})")
