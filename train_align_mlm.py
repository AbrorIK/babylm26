...
# ---- ALIGN-MLM specific arguments ----
parser.add_argument("--align_dict_en_nl", type=str, default="data/en-nl.txt",
                    help="Path to MUSE EN-NL dictionary")
parser.add_argument("--align_dict_en_zh", type=str, default="data/en-zh.txt",
                    help="Path to MUSE EN-ZH dictionary")
parser.add_argument("--align_alpha", type=float, default=10.0,
                    help="Weight for alignment loss (alpha). Tang et al. use 10.")
parser.add_argument("--align_batch_size", type=int, default=256,
                    help="Number of dictionary pairs sampled per training step")
parser.add_argument("--align_eval_steps", type=int, default=0,
                    help="How often to compute alignment accuracy (0 = same as eval_steps)")


# ------------------------------------------------------------------ #
# Shared utilities (same as train_mask.py)                           #
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
# ALIGN-MLM Training Loop                                           #
# ------------------------------------------------------------------ #

def train(args, model, tokenizer, train_dataloader, eval_dataloader, dict_pairs):
    if args.flops:
    ...
    # Resolve alignment eval frequency
    align_eval_steps = args.align_eval_steps if args.align_eval_steps > 0 else args.eval_steps
    ...
    print(f"Alignment alpha: {args.align_alpha}")
    print(f"Alignment batch size: {args.align_batch_size}")
    print(f"Dictionary pairs: {len(dict_pairs):,}")
    with tqdm(total=args.total_steps) as pbar:
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # ---- Job 2: Alignment loss (on embedding table, no forward pass) ----
                # Computed once per optimizer step (not per mini-batch) to avoid
                # over-weighting with large grad_acc.
                dict_batch = sample_dict_batch(dict_pairs, k=args.align_batch_size)
                a_loss = align_loss_batch(model, dict_batch)
                align_loss_val = a_loss.item()

                # Scale by alpha and backprop through embeddings
                (args.align_alpha * a_loss).backward()
                ...
                if is_step("logging", global_step, args):
                    ...
                    mlm_loss_avg = mlm_loss_accum / args.grad_acc
                    total_loss = mlm_loss_avg + args.align_alpha * align_loss_val
                    print(
                        f"Epoch {epoch_float:.2f}, "
                        f"MLM Loss: {mlm_loss_avg:.4f}, "
                        f"Align Loss: {align_loss_val:.4f}, "
                ...
                # ----- ALIGNMENT ACCURACY -----
                if global_step > 0 and global_step % align_eval_steps == 0:
                    align_metrics = compute_alignment_accuracy(model, dict_pairs, k=5000)
                    print(
                        f"----- Alignment accuracy: {align_metrics['align_acc']:.2f}%, "
                        f"Avg cosine: {align_metrics['avg_cosine']:.4f} -----",
                        flush=True,
                    )
                    if args.wandb:
                        wandb.log(align_metrics)
    ...
    align_metrics = compute_alignment_accuracy(model, dict_pairs, k=5000)
    print(
        f"Final alignment accuracy: {align_metrics['align_acc']:.2f}%, "
        f"Avg cosine: {align_metrics['avg_cosine']:.4f}",
        flush=True,
    )
...
def main():
    ...
    # ---- No code-switching control tokens for ALIGN-MLM ----
    # Tang et al. deliberately omit language-ID embeddings because they
    # interfere with aligning the embedding space cleanly (Step 2).
    ...
    # ---- Load alignment dictionaries (Step 1) ----
    print("Loading alignment dictionaries...")
    dict_pairs = load_all_dictionaries(
        {
            "en-nl": args.align_dict_en_nl,
            "en-zh": args.align_dict_en_zh,
        },
        tokenizer,
    )

    # ---- Load MLM training data ----
    ...
    train(args, model, tokenizer, train_dataloader, eval_dataloader, dict_pairs)
